...
const SharedForm = ({ formData, handleChange, handleSubmit, handleBack, loading, errors }) => {
  const [actions, setActions] = useState([
    { action: 'Input Field', dataItem: '', value: '' },
  ]);

  const handleAddAction = () => {
    setActions([...actions, { action: 'Input Field', dataItem: '', value: '' }]);
  };
  ...
  const isFormValid = () => {
    return actions.every(action => {
      ...
      return action.dataItem.trim() !== '' && action.value.trim() !== '';
    });
  };
      ...
      <form onSubmit={handleSubmit} style={{ marginTop: '20px' }}>
        {actions.map((action, index) => (
          <Grid container spacing={2} key={index} style={{ marginTop: '10px' }}>
            ...
            <Grid item xs={12} sm={5}>
              {action.action === 'Sleep' ? (
                ...
                <TextField
                  ...
                  onChange={(e) => handleActionChange(index, 'dataItem', e.target.value)}
                  InputProps={{
            ...
            {(action.action === 'Input Field' || action.action === 'Set Dropdown') && action.action !== 'Sleep' && (
              <Grid item xs={12} sm={5}>
                <TextField
                  fullWidth
                  margin="normal"
